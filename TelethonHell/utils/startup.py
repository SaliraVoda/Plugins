    ...
    try:
        grp = await client(
            functions.channels.CreateChannelRequest(
                title="HELLEXTENDED LOGGER ⚡", about=desc, megagroup=True
            )
        )
...
<b><i>»» <u><a href='https://t.me/hellbot_extended'>†hê Hêllẞø†</a></u> ««</i></b>
"""
    await client.send_file(
        ...
        caption=text,
        parse_mode="HTML",
        buttons=[[Button.url("HellBot Network", "https://t.me/HellBot_Extended")]],
    )
...
async def join_it(client):
    if client:
        try:
            await client(functions.channels.JoinChannelRequest("@HELLBOT_EXTENDED"))
